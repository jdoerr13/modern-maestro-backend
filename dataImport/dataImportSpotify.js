...
async function processAndInsertData(tracks, composerNameOverride = null) {
  ...
  for (const track of tracks) {
    ...
    if (!existingComposition) {
      ...
      console.log(`✅ Inserted: ${compositionData.title}`);
    } else {
      console.log(`⏭️ Skipped duplicate: ${existingComposition?.title || 'Untitled'}`);
    }
  }
}
...
(async () => {
  try {
    ...
    // Optional:
    // const playlistId = '4hOKQuZbraPDIfaGbM3lKI';
